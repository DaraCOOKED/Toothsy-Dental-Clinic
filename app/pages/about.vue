<template>
  <div class="relative isolate overflow-x-hidden bg-[#EFF8FC]">
    ...
    <!-- ════════════════════════════════════════════════════
      ...
      Fixed/absolute, pointer-events-none, z-0
      All drift independently throughout the full page
    ════════════════════════════════════════════════════ -->
        ...
        <div ref="specialistsRef" class="bg-[#EFF8FC] -mx-2 px-2 md:px-5 py-10">
          ...
          <div class="max-w-5xl mx-auto">
            <div
              ...
              class="grid grid-cols-1 sm:grid-cols-[180px_1fr] md:grid-cols-[220px_1fr]
                     gap-6 md:gap-8 mb-12 items-start will-change-transform doctor-reveal"
              :class="doctorVisible[i] ? 'is-visible' : ''"
              :style="{ transitionDelay: doctorVisible[i] ? `${i * 100}ms` : '0ms' }"
            >
              <div class="rounded-xl overflow-hidden h-56 sm:h-64 w-full sm:w-[180px]
                          md:w-[220px] group relative mx-auto sm:mx-0">
                <img :src="doctor.photo" :alt="doctor.name"
                  class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"/>
                <div class="absolute inset-0 bg-[#5bbfa0]/0 group-hover:bg-[#5bbfa0]/40 transition-all duration-500"/>
                <div class="absolute bottom-0 left-0 right-0 bg-[#5bbfa0] text-white text-xs
                            font-sans px-3 py-2 translate-y-full group-hover:translate-y-0 transition-transform duration-500">
                  {{ doctor.name }}
                </div>
              </div>
              <div class="text-center sm:text-left">
                <h3 class="text-lg md:text-xl font-bold text-gray-900">
                  {{ doctor.name }}
                  <span class="font-normal text-sm text-gray-500 ml-2">( {{ doctor.specialty }} )</span>
                </h3>
                <p class="text-sm leading-7 text-gray-600 font-sans my-3">{{ doctor.bio }}</p>
                <button class="bg-[#5bbfa0] text-white rounded-full px-6 py-2.5 text-sm font-sans
...
const doctors = [
  ...
  },
  {
    ...
    specialty: 'Specialty in ...',
    photo: drFourthImg,
    bio: 'Bio for the fourth doctor...'
  }
]
...
function applyToDOM() {
  ...
  if (blob2Ref.value)
    blob2Ref.value.style.transform = `translate3d(${s.b2X}px,${s.b2Y}px,0)`
  if (blob3Ref.value)
  ...
  if (accentSquareRef.value)
    accentSquareRef.value.style.transform = `translate3d(${s.asX}px,${s.asY}px,0) rotate(${s.asR}deg)`
  if (dotGridRef.value)
    dotGridRef.value.style.transform = `translate3d(0,${s.dgY}px,0)`
  if (accentRing2Ref.value)
    accentRing2Ref.value.style.transform = `translate3d(0,${s.ar2Y}px,0) rotate(${s.ar2R}deg)`
...
function setupReveal() {
  const targets = [
    ...
    ...serviceCardEls.value.filter(Boolean).map((el, i) => ({
      el, cb: () => { serviceVisible[i] = true }
    })),
  ].filter(t => t.el)

  if (prefersReducedMotion) { targets.forEach(t => t.cb()); return }
  ...
  revealObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach(entry => {
        ...
        if (match) { match.cb(); revealObserver.unobserve(entry.target) }
      })
    },
    { threshold: 0.12, rootMargin: '0px 0px -6% 0px' }
  )
  targets.forEach(t => revealObserver.observe(t.el))
}
